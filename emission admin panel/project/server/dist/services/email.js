...
export const sendEmail = async (to, type, variables = {}) => {
    try {
        ...
        if (error.code === 'EENVELOPE')
            console.error('  - Possible cause: Invalid sender or recipient address.');
        if (error.code === 'ETIMEDOUT')
            console.error('  - Possible cause: Firewall blocking outgoing port or wrong SMTP host.');
        if (error.code === 'EAUTH')
            console.error('  - Possible cause: SMTP authentication failed. Check credentials.');
        return false;
    }
};
